# ==========================================
# Streamlit Multi-Agent DQN ET Jamming Dashboard (with PW & PRI)
# ==========================================
...
# -------------------------------
# Realistic Jamming Environment (PW & PRI included)
# -------------------------------
    ...
    def reset(self, seed=None, options=None):
        ...
        for agent in self.agents:
            if "radar" in agent:
                ...
                self.state[agent] = np.zeros(5, dtype=np.float32)
        ...
        for agent, action in action_dict.items():
            ...
            if "jammer" in agent:
                for r in [a for a in self.agents if "radar" in a]:
                    radar_freq = int(self.state[r][1])
                    radar_PRI = self.state[r][3]
                    radar_PW = self.state[r][4]
                    # SNR düşüşünü güç + PW + PRI farklılığı ile hesapla
                    if freq == radar_freq:
